...
namespace McpServer.TokenExchange.Tools;
...
public sealed class TaskTools
{
    ...
    {
        var subjectToken = GetBearerToken()
            ?? throw new McpException("No bearer token available. Please authenticate first.");

        // RFC 8693 Token Exchange: swap the user's token for one scoped to project-api
        var exchangedToken = await _tokenExchangeService.ExchangeTokenAsync(subjectToken);
        ...
        if (response.StatusCode == System.Net.HttpStatusCode.Unauthorized)
            throw new McpException("Authentication failed. The exchanged token may be invalid or expired.");
